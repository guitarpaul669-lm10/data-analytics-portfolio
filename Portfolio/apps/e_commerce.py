...
if auswahl == "1. Welche Produktkategorien sind am beliebtesten?":
    ...
    ORDER BY percent DESC;
        ...
        LEFT JOIN orders ord ON ord.customer_id = cu.customer_id
        LEFT JOIN payment pa ON pa.order_id = ord.order_id
        GROUP BY cu.customer_unique_id, cu.customer_id
        ORDER BY summe_bestellungen DESC
        LIMIT 20
        ) t
        )
        ...
        LEFT JOIN products pr ON it.product_id = pr.product_id
        LEFT JOIN orders ord ON ord.order_id = it.order_id
        JOIN top_summe ts ON ts.customer_id = ord.customer_id
        ...
        SELECT cu.customer_unique_id,
        SUM(payment_value) AS summe_bestellungen
        FROM customer cu
        LEFT JOIN orders ord ON ord.customer_id = cu.customer_id
        LEFT JOIN payment pa ON pa.order_id = ord.order_id
        JOIN top_anzahl tp ON tp.customer_id = ord.customer_id
        ...
        url_2 = "https://drive.google.com/uc?export=download&id=1FBPEithizEwd4PHdmOZ0i0p7ajZyz7NY"
        url_3 = "https://drive.google.com/uc?export=download&id=1x9meeDY_6iGrzhNlOM-MYNDpuNsw2cbw"
        df = pd.read_csv(url)
        cust_geo = pd.read_csv(url_1)
        top_summe_produkt = pd.read_csv(url_3)
        top_anzahl_produkt = pd.read_csv(url_2)
        ...
        if wahl == "Top Kunden nach Bestellwert":
                ...
                top_summe_produkt["product_percent"] = top_summe_produkt["product_count"] / top_summe_produkt["product_count"].sum() * 100
                fig = px.bar(top_summe_produkt, x = "product_category_name", y = "product_percent", labels={"product_percent": "Anteil (%)", "product_category_name": "Produktkategorie"},title="Produktkategorien der Top Kunden")
                st.plotly_chart(fig)
                st.subheader("Verwendete SQL-Query")
                ...
                fig.add_vline(x=45.56, line_width = 3, line_dash = "dash", line_color = "green", annotation_text = "Threshold der Top 20 Kunden", annotation_position = "top right")
                fig.add_vline(x=1255.71, line_width = 3, line_dash = "dash", line_color = "red", annotation_text = "Maximale Summe der Bestellungen eines Top Kundens", annotation_position = "top right")
                st.plotly_chart(fig)
                st.markdown("Der Threshold und die maximale Bestellungssumme der Top Kunden (nach der Bestellanzahl) liegt weit unten in der Verteilung. Dementsprechend bestellen viele Kunden weniger, aber dafür preiswertere Produkte.  \n"
                            "**Das deutet darauf hin das ein erheblicher Teil von Gelegenheitskäufern kommen, die teure Produkte erwerben.**")
                st.subheader("Verwendete SQL-Query")
                st.code(sql_query_6, language="sql")
                st.subheader("Herkunft der Kunden")
                top_cust_geo = cust_geo[cust_geo["customer_unique_id"].isin(top_anzahl)]
                customers = go.Scattermapbox(
                lat=cust_geo["cust_lat"],
                lon=cust_geo["cust_lng"],
                mode="markers",
                ...
                st.markdown("**Der Großteil der Top Kunden kommen aus den Großstädten Brasiliens.**")
                st.subheader("Verwendete SQL-Query")
                st.code(sql_query_7, language = "sql")
                st.subheader("Produktkategorien")
                top_anzahl_produkt["product_percent"] = top_anzahl_produkt["product_count"] / top_anzahl_produkt["product_count"].sum() * 100
                fig = px.bar(top_anzahl_produkt, x = "product_category_name", y = "product_percent", labels={"product_percent": "Anteil (%)", "product_category_name": "Produktkategorie"},title="Produktkategorien der Top Kunden")
                st.plotly_chart(fig)
                st.markdown("**Die Produktkategorien bestätigen die oben genannte These. Die Produktkategorien erscheinen eher preiswert.**")
                st.subheader("Verwendete SQL-Query")
